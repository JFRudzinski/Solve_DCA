...
dtrajs_3nn_3456 = []
for i in range( len(dtraj_rama_2) ):
    dtrajs_3nn_2345.append( np.vstack( (dtrajs_rama_2[i], dtrajs_rama_3[i], dtrajs_rama_4[i], dtrajs_rama_5[i]) ).T )
    dtrajs_3nn_2345[i].astype('int64')
    dtrajs_3nn_3456.append( np.vstack( (dtrajs_rama_3[i], dtrajs_rama_4[i], dtrajs_rama_5[i], dtrajs_rama_6[i]) ).T )
    dtrajs_3nn_3456[i].astype('int64')
    

# In[18]:

n_clusters = 16
clustering_3nn_2345 = coor.cluster_regspace(dtrajs_3nn_2345,max_centers=n_clusters,dmin=0.5)
clustering_3nn_3456 = coor.cluster_regspace(dtrajs_3nn_3456,max_centers=n_clusters,dmin=0.5)

# In[19]:

dtrajs_1D_2345 = clustering_3nn_2345.dtrajs
dtrajs_1D_3456 = clustering_3nn_3456.dtrajs

# In[20]:

# shift the cluster indices so they are all consistent
cc_2345 = clustering_3nn_2345.clustercenters[:]
cc_3456 = clustering_3nn_3456.clustercenters[:]

# needed more complex sorting method for ipm, m>1
# calculate the number of adjacent and almost adjacent alphas for each state
cc_nn_2345 = np.zeros(len(cc_2345))
cc_nnn_2345 = np.zeros(len(cc_2345))
for i in range(len(cc_2345)):
    if ( np.where(cc_2345[i]==1)[0].shape[0] == 2):
        d1 = np.where(cc_2345[i]==1)[0][1] - np.where(cc_2345[i]==1)[0][0]
        if (d1==1):
            cc_nn_2345[i] += 1
        elif (d1==2):
            cc_nnn_2345[i] += 1
    elif ( np.where(cc_2345[i]==1)[0].shape[0] == 3):
        d1 = np.where(cc_2345[i]==1)[0][1] - np.where(cc_2345[i]==1)[0][0]
        d2 = np.where(cc_2345[i]==1)[0][2] - np.where(cc_2345[i]==1)[0][1]
        if (d1==1):
            cc_nn_2345[i] += 1
        elif (d1==2):
            cc_nnn_2345[i] += 1
        if (d2==1):
            ...
            cc_nnn_2345[i] += 1
    elif ( np.where(cc_2345[i]==1)[0].shape[0] == 4):
        d1 = np.where(cc_2345[i]==1)[0][1] - np.where(cc_2345[i]==1)[0][0]
        d2 = np.where(cc_2345[i]==1)[0][2] - np.where(cc_2345[i]==1)[0][1]
        d3 = np.where(cc_2345[i]==1)[0][3] - np.where(cc_2345[i]==1)[0][2]
        if (d1==1):
            cc_nn_2345[i] += 1
        elif (d1==2):
            cc_nnn_2345[i] += 1
        ...
        if (d3==1):
            ...
            cc_nnn_2345[i] += 1
#
cc_nn_3456 = np.zeros(len(cc_3456))
cc_nnn_3456 = np.zeros(len(cc_3456))
for i in range(len(cc_3456)):
    if ( np.where(cc_3456[i]==1)[0].shape[0] == 2):
        d1 = np.where(cc_3456[i]==1)[0][1] - np.where(cc_3456[i]==1)[0][0]
        if (d1==1):
            cc_nn_3456[i] += 1
        elif (d1==2):
            cc_nnn_3456[i] += 1
    elif ( np.where(cc_3456[i]==1)[0].shape[0] == 3):
        d1 = np.where(cc_3456[i]==1)[0][1] - np.where(cc_3456[i]==1)[0][0]
        d2 = np.where(cc_3456[i]==1)[0][2] - np.where(cc_3456[i]==1)[0][1]
        if (d1==1):
            cc_nn_3456[i] += 1
        elif (d1==2):
            cc_nnn_3456[i] += 1
        if (d2==1):
            ...
            cc_nnn_3456[i] += 1
    elif ( np.where(cc_3456[i]==1)[0].shape[0] == 4):
        d1 = np.where(cc_3456[i]==1)[0][1] - np.where(cc_3456[i]==1)[0][0]
        d2 = np.where(cc_3456[i]==1)[0][2] - np.where(cc_3456[i]==1)[0][1]
        d3 = np.where(cc_3456[i]==1)[0][3] - np.where(cc_3456[i]==1)[0][2]
        if (d1==1):
            cc_nn_3456[i] += 1
        elif (d1==2):
            cc_nnn_3456[i] += 1
...
cc_stack_3456 = []
for i in range(len(cc_2345)):
    cc_stack_2345.append(np.hstack((np.sum(cc_2345,axis=1)[i],cc_nn_2345[i],cc_nnn_2345[i],cc_2345[i],lcc[i])))
    cc_stack_3456.append(np.hstack((np.sum(cc_3456,axis=1)[i],cc_nn_3456[i],cc_nnn_3456[i],cc_3456[i],lcc[i])))

# In[22]:

sorted_list_2345 = sorted(cc_stack_2345, key=operator.itemgetter(0,1,2,4,5,3,6))
sorted_list_3456 = sorted(cc_stack_3456, key=operator.itemgetter(0,1,2,4,5,3,6))
cc_sorted_2345 = np.array(sorted_list_2345)[:,3:7]
cc_sorted_3456 = np.array(sorted_list_3456)[:,3:7]
cc_sorted_2345 = cc_sorted_2345.tolist()
cc_sorted_3456 = cc_sorted_3456.tolist()
for i in range(len(cc_2345)):
    cc_sorted_2345[i] = map(int,cc_sorted_2345[i])
    cc_sorted_3456[i] = map(int,cc_sorted_3456[i])

# In[23]:

lcc_sorted_2345 = np.array(sorted_list_2345)[:,7]
lcc_sorted_3456 = np.array(sorted_list_3456)[:,7]
lcc_sorted_2345 = lcc_sorted_2345.tolist()
lcc_sorted_3456 = lcc_sorted_3456.tolist()
lcc_sorted_2345 = map(int, lcc_sorted_2345)
lcc_sorted_3456 = map(int, lcc_sorted_3456)

# In[24]:

dtrajs_1D_2345_sorted = []
dtrajs_1D_3456_sorted = []
for i in range( dtrajs_1D_2345[0].shape[0] ):
    dtrajs_1D_2345_sorted.append(lcc_sorted_2345[dtrajs_1D_2345[0][i]])
    dtrajs_1D_3456_sorted.append(lcc_sorted_3456[dtrajs_1D_3456[0][i]])

# In[25]:

lags = np.linspace(1,1000,200,dtype='int')
its_2345 = msm.timescales_msm(dtrajs_1D_2345_sorted, lags=lags, nits=n_clusters)
its_3456 = msm.timescales_msm(dtrajs_1D_3456_sorted, lags=lags, nits=n_clusters)

# In[27]:

tau = 400
Cmat_2345 = pyemma.msm.estimation.count_matrix(dtrajs_1D_2345_sorted, tau, sliding=True, sparse_return=False)
Cmat_3456 = pyemma.msm.estimation.count_matrix(dtrajs_1D_3456_sorted, tau, sliding=True, sparse_return=False)

# In[28]:

# Treat the quads independently
#Cmat_totind = Cmat_2345+Cmat_3456

# In[29]:

lcc_cc_2345 = pyemma.msm.estimation.largest_connected_set(Cmat_2345, directed=True)
Cmat_cc_2345 = pyemma.msm.estimation.largest_connected_submatrix(Cmat_2345, directed=True, lcc=lcc_cc_2345)
Tmle_cc_2345 = pyemma.msm.estimation.transition_matrix(Cmat_cc_2345, reversible=True)
lcc_cc_3456 = pyemma.msm.estimation.largest_connected_set(Cmat_3456, directed=True)
Cmat_cc_3456 = pyemma.msm.estimation.largest_connected_submatrix(Cmat_3456, directed=True, lcc=lcc_cc_3456)
Tmle_cc_3456 = pyemma.msm.estimation.transition_matrix(Cmat_cc_3456, reversible=True)
...
# 4res
np.save('lcc4res',lcc_sorted_2345)
np.save('cc4res',cc_2345)
np.save('T4res_0',Tmle_cc_2345)
np.save('T4res_1',Tmle_cc_3456)
# full
