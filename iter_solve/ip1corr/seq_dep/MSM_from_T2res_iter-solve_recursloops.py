...
for pair in range(Npairs):
    P2res_inp.append(np.load('T2res_'+str(pair)+'.npy'))
...
mu_eff_1res = []
for pair in range(Npairs):
    mle_tmp = pyemma.msm.markov_model(P2res_inp[pair])
    mu_eff_2res.append(mle_tmp.eigenvectors_left(k=1)[0])
for res in range(Nres):
    mu_eff_1res.append(np.zeros(2))
    for state in range(len(states_2res)):
        ind1 = states_2res[state][0]
        ind2 = states_2res[state][1]
        if ( res == 0 ):
            mu_eff_1res[res][ind1] += mu_eff_2res[res][state]
        elif (res == Nres-1):
            mu_eff_1res[res][ind2] += mu_eff_2res[res-1][state]
        else:
            mu_eff_1res[res][ind1] += 0.5*mu_eff_2res[res][state]
            mu_eff_1res[res][ind2] += 0.5*mu_eff_2res[res-1][state]
...
for res in range(Nres-1):
    for s1 in range(len(states_2res)):
        for s2 in range(len(states_2res)):
            var = P2res[res][s1][s2]
            inp_var[var] = P2res_inp[res][s1][s2]
# P1res, boundary only
for res in range(2):
    for si in range(len(S2)):
        for sip in range(len(S2)):
...
def add_P2res_stat_dict_vals( dic, P2res_stat, P2res_stat_val, states_2res ):
    for res in range(Nres-1):
        for state in range(len(states_2res)):
            ...
            dic[var] = P2res_stat_val[res][state]
    return dic
...
for res in range(Nres):
    for state in range(2):
        ...
        inp_tmp[var] = mu_eff_1res[res][state]
