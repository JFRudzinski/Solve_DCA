...
# Below, I set the states by hand for 3 residues.
#state1 = 0
#state2 = 0
#state3 = 0
...
for res in range(Nres):
    for si in range(len(S2)):
                ...
                for sip1p in range(len(S2)):
                    for sim1 in range(len(S2)):
                        for sip1 in range(len(S2)):
                            ...
                            if ( res == 0 ):
                                pxpx[sip] += P2res_end[0][ind1][ind1p]*P2res[res][ind2][ind2p]*P2res_stat_end[0][ind1]*P2res_stat[res][ind2]
                            elif ( res == Nres-1 ):
                                pxpx[sip] += P2res[res-1][ind1][ind1p]*P2res_end[1][ind2][ind2p]*P2res_stat[res-1][ind1]*P2res_stat_end[1][ind2]
		            else:
        ...
        eqns_P1res.append( P1res[res][si][0]**2 - (pxpx[0] / np.sum(pxpx)) )
        eqns_P1res.append( P1res[res][si][1]**2 - (pxpx[1] / np.sum(pxpx)) )
        # also add the 1 res static definitions for each res
        eqns_P1res_stat.append( P1res_stat[res][si] - np.sum(pxpx) )

# definitions of the 2res static probabilties
eqns_P2res_stat = []
for res in range(Nres-1):
    for si in range(len(S2)):
        for sip1 in range(len(S2)):
...
# not sure if eqns or direct substitutions is better
eqns_T2res_inp = []
...
for res in range(2):
    for s1 in range(len(states_2res)):
        ...
        if ( res == 0 ):
            eqns_bndry_cond.append( var - P1res_stat[0][states_2res[s1][1]] )
        else:
            eqns_bndry_cond.append( var - P1res_stat[Nres-1][states_2res[s1][0]] )
...
for res in range(-1,Nres):
    for s1 in range(len(states_2res)):
        if ( res == -1 ):
            var = P2res_stat_end[0][s1]
            if ( s1 == 0 ):
                inp_var[var] = 0.
            else:
                inp_var[var] = 1.
        elif ( res == Nres-1 ):
            var = P2res_stat_end[1][s1]
            if ( s1 == 0 ):
                inp_var[var] = 0.
            else:
                inp_var[var] = 1.            
