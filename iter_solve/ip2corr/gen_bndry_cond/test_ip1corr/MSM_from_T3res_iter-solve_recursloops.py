    ...
    for state in states:
        ...
        for statep in statesp:
            ...
            for kset in range(Nres_corr-Ncorr):
                ...
                if ( (res<Ncorr) and (kset<Ncorr+1-res) ):
                ...
                elif ( (res>Nres-1-Ncorr-1) and (kset>(Nres_corr-Ncorr-1)-( res-(Nres-Ncorr-1)+1 )) ):  
                    # some useful quantities
                    # pos_ind = pos_start + kset - kmin
                    # Nksets = res-(Nres-Ncorr-1)+1, pos_start = Nres-1-Ncorr
                    # dkset = kset - ksetmax, posmax = Nres-Ncorr, dposmax = posmax-res
                    # ksetmax = Nres_corr-Ncorr-1, kmin = kmax-Nksets+1
                    pos_start = (Nres-1-Ncorr)
                    kmax = (Nres_corr-Ncorr-1)
                    Nksets = (res-(Nres-Ncorr-1)+1)
                    kmin = ( kmax - Nksets + 1 )
                    kmid = Ncorr
                    pos_ind = pos_start + kset - kmin 
                    tmp *= Pkp1res_end[pos_ind][ind][indp]*Pkp1res_stat_end[pos_ind][ind] 
                    if ( (kset > kmin) and (kset != kmid) ):
                        if ( res+kset > Nres ):
                            tmp /= (Pkres_end[pos_ind][ind2][ind2p]*Pkres_stat_end[pos_ind][ind2])
                        else:
                    ...
                    if ( (kset!=0) and (kset!=Ncorr) and (kset!=(Nres_corr-Ncorr-1)) ):
                        tmp /= (Pkres[res-Ncorr+kset][ind2][ind2p]*Pkres_stat[res-Ncorr+kset][ind2])
